...
クライアント側のログアウト操作では、デモセッション確認後に `/auth/demo` へ `DELETE` を送り、通常の Supabase セッションでは `supabase.auth.signOut()` を呼ぶ。
...
`/auth/signout` はサーバー側でセッション Cookie を失効させたい場合の POST endpoint として残す。GET は CSRF 回避のため受け付けない。
