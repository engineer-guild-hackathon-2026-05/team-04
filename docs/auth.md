...
2. Supabase Dashboard / CLI で取得した Project URL と publishable key を `.env.local` に設定
...
`NEXT_PUBLIC_` 付きの値はブラウザに露出する。publishable keyはRLS前提の公開キーとして扱い、ユーザー本人の操作はCookieセッション + RLSで制限する。
秘密鍵（secret key など）はブラウザ向け環境変数に入れない。
