...
assert.match(demoBranch, /saveDemoProfile\(demoSessionId, \{[\s\S]*userName: requestedUserName[\s\S]*restrictedIngredients: requestedRestrictedIngredients[\s\S]*restrictedIngredientReasons: requestedRestrictionReasons[\s\S]*preferredDishes[\s\S]*preferredCuisines/s, 'demo PUT は送信payloadをDB保存関数へ渡してください。');
