...
assert.match(authDemoRoute, /if \(!isDemoPersistenceConfigured\(\)\) \{[\s\S]*createUnavailableResponse\(\)/, 'demo login はDB永続化なしでmock認証へfallbackしないでください。');
...
assert.match(envExample, /DB-backed デモログイン[\s\S]*SUPABASE_SERVICE_ROLE_KEY[\s\S]*DEMO_SESSION_SECRET[\s\S]*DEMO_MODE=false/, '.env.local.example はDB-backed demo loginに必要なenvを案内してください。');
