...
assert.equal(fixedClientAuth({ demoSession: 'unauthenticated', hasProfileApiSession: true }), 'authenticated');
assert.doesNotMatch(
  source,
  /if\s*\(demoSession\s*===\s*'unauthenticated'\s*\|\|\s*demoSession\s*===\s*'failed'\)\s*\{[\s\S]*?setAuthStatus\('unauthenticated'\);[\s\S]*?return;/,
);
assert.match(source, /if\s*\(demoSession\s*===\s*'failed'\)\s*\{[\s\S]*console\.error\(/);
