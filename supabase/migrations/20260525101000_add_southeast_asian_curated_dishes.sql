...
with ingredient_seed as (
  select distinct
    ...
    coalesce(nullif(item ->> 'category', ''), '世界料理') as category,
    array(
      select jsonb_array_elements_text(coalesce(item -> 'dietary_tags', '[]'::jsonb))
    ) as dietary_tags
  from _southeast_asian_dish_seed d
...
insert into public.ingredients (name_ja, name_en, category, dietary_tags)
select name_ja, name_en, category, coalesce(s.dietary_tags, array[]::text[])
